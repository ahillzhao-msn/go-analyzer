"""
GoStrengthModel v2 — 围棋定制架构。

v1 → v2 改进:
  1. 黑白分离 SelfAttn → 棋手视角，非上帝视角
  2. Causal Masking → 一手棋不看未来
  3. 剩余自由度编码 → 官子可走位置越来越少
  4. Cosine Cross-Attention → 复盘式黑白交互
  5. Attention Pooling → 模型自己学哪些位置重要
  6. 全环境向量融合 (Global+HW+SW+Game)
"""
